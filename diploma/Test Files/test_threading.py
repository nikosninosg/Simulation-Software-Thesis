...
# -------------------------------------------------
# Dataframe Pandas
data = [[0, 'STARTUP', 'STARTUP', 'STARTUP']]
...
def break_and_repair_machine(machine, repairers):
    ...
    if random.uniform(1, 1000) == 2:  # probability 1/30000
        df.append({'ENV_NOW': env.now, machine: 'RED'}, ignore_index=True)
        ...
        with repairers.request() as request:
            yield request
            rep_time = random.gauss(mu=15, sigma=5)
...
class CanPackLine(object):
    ...
    def __init__(self):
        ...
        # RAW MATERIAL
        self.emptyCan = simpy.Container(env, capacity=depall_capacity, init=0)
        ...
        while True:
            ...
            if self.filledCans.level < pasteur_min_input:
                ...
                print('%.2f: ' % env.now + 'Filled Cans stock = {0} bellow critical level < {1} at {2}'.format(
                    self.filledCans.level, pasteur_critical_buffer, round(env.now, 2)))
                displayAlert(
                    "O Pasteur έχει τεθεί σε κατάσταση Stand By (Status = ORANGE).\n Αιτία: Δεν έχει κουτιά να παράξει. ")
                write_row({'Env_now': env.now, 'Machine': 'Depall',
                           'Description': 'Δεν υπάρχουν filledCans κουτιά για να παστεριωθούν.',
...
def depall():
    ...
    while True:
        if can_pack_line.emptyCan.level >= depall_batch:
            ...
            df.append({'ENV_NOW': env.now, 'Depall': 'GREEN'}, ignore_index=True)
            print(
            ...
            #           'Init_level': initial_depallCans, 'Capacity': depall_capacity, 'Status': 'GREEN'})
...
def filler():
    ...
    while True:
        if can_pack_line.depallCans.level >= filler_batch:
            ...
            #           'Init_level': initial_filledCans, 'Capacity': filler_capacity, 'Status': 'GREEN'})
            print(
...
def pasteur():
    ...
    while True:
        ...
        if can_pack_line.filledCans.level >= pasteur_min_input:
            ...
            # write_row({'Env_now': env.now, 'Machine': 'Pasteur', 'Description': 'Before Process', 'Current_cans': can_pack_line.pastCans.level,
            #           'Init_level': initial_pastCans, 'Capacity': pasteur_capacity, 'Status': 'GREEN'})
            print(
            ...
            # STATUS ORANGE
            df.append({'ENV_NOW': env.now, 'Pasteur': 'ORANGE'}, ignore_index=True)
            yield env.timeout(2)
...
print("depall process total time = %f seconds" % round(sum(depall_total_ptime), 2))
