...
depall_ptime = 12  # Depall Process Time
depall_input = 1000  # Πόσο προϊόν εισάγει για να επεξεργαστεί (input cans/min)
depall_output = 1000  # Πόσο προϊόν εξάγει μετά την επεξεργασία (output cans/min)
...
filler_critical_buffer = filler_capacity / 2
filler_ptime = 5  # Filler Process Time
filler_input = 1000  # Πόσο προϊόν εισάγει για να επεξεργαστεί (input cans/min)
filler_output = 1000  # Πόσο προϊόν εξάγει μετά την επεξεργασία (output cans/min)
MTBF_filler = 5.34  # standard error se min (MTTF)

# PASTEUR
pasteur_capacity = 10000000  # Filled cans που μπορούν να περιμένουν μέχρι να επεξεργαστούν
initial_pastCan = 0  # Number αρχικών Filled can pou έχει ο Pasteur
pasteur_critical_buffer = pasteur_capacity / 2
pasteur_ptime = 16  # Pasteur Process Time
pasteur_input = 1000  # Πόσο προϊόν εισάγει για να επεξεργαστεί (input cans/min)
pasteur_output = 1000  # Πόσο προϊόν εξάγει μετά την επεξεργασία (output cans/min)
...
class CanPackLine(object):
    ...
    def __init__(self):
        ...
        self.fillerCanControl = env.process(self.filler_can_stock_control())
        ...
        # self.pasteurCanControl = env.process(self.pasteur_can_stock_control())
...
def depall():
    while True:
        yield env.timeout(1)  # 1 min διαρκεί να μπουν τα κουτιά στο depall
        print(
        ...
        write_row({'Env_now': env.now, 'Machine': 'Depall', 'Description': 'Before Process',
                   'Current_cans': can_pack_line.palletCan.level,
                   'Init_level': initial_depallCan, 'Capacity': depall_capacity, 'Status': 'GREEN'})
        yield can_pack_line.palletCan.get(depall_input)
        ...
        yield can_pack_line.depallCan.put(depall_output)
...
def filler():
    while True:
        ...
        write_row({'Env_now': env.now, 'Machine': 'Filler', 'Description': 'Before Process',
                   'Current_cans': can_pack_line.filledCan.level,
        ...
        yield can_pack_line.depallCan.get(filler_input)
        print(
        ...
        yield can_pack_line.filledCan.put(filler_output)
...
def pasteur():
    while True:
        ...
        write_row({'Env_now': env.now, 'Machine': 'Pasteur', 'Description': 'Before Process',
                   'Current_cans': can_pack_line.pastCan.level,
        ...
        yield can_pack_line.filledCan.get(pasteur_input)
        print(
        ...
        print('%d: ' % env.now + Fore.LIGHTCYAN_EX + "Pasteur P Finish: Γέμισε {0} κουτιά τη στιγμή {1}".format(1000,
                                                                                                                env.now) + Fore.RESET)
        yield can_pack_line.pastCan.put(pasteur_output)
        print(
...
print(
    "Συνολικά έχουν παραχθεί: " + str(can_pack_line.dispatch.level) + " κουτιά μπύρας σε " + str(WEEKS) + " εβδομάδες")
