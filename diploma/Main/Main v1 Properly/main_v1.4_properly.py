...
# Parameters
# MANUAL_SIM_TIME, SHIFT, DT = welcomeScreen()
...
def break_and_repair_machine(machine, repairers):
    ...
    if np.random.uniform(low=min_prob, high=max_prob) == 2:  # probability 1/30000
        ...
        yield env.timeout(1)  # 1 time unit to detect broke
...
class CanPackLine(object):
    ...
    def __init__(self):
        ...
        # self.live_monitoring_gen = env.process((live_monitoring()))
...
def depall():
    ...
    while True:
        if can_pack_line.emptyCan.level >= depall_batch:
            ...
            print('%.2f: ' % env.now + Fore.BLUE + "Empty Cans Before proc: %d" % can_pack_line.emptyCan.level + Fore.RESET)
            ...
            print('%.2f: ' % env.now + Fore.BLUE + "Empty Cans After proc: %d" % can_pack_line.emptyCan.level + Fore.RESET)
...
def live_monitoring():
    ...
    # Depall
    l1 = Label(bg=depall_status, bd=4, height=7, justify=LEFT, padx=100, relief=RAISED, text='Depall', font="Arial 15 bold ")
    l1.grid(row=1, column=0, padx=100, sticky=W)

    # Filler
    l2 = Label(bg=filler_status, bd=4, height=7, justify=CENTER, padx=100, relief=RAISED, text='Filler', font="Arial 15 bold ")
    l2.grid(row=1, column=1, padx=100, pady=30, sticky=N)

    # Pasteur
    l3 = Label(bg=pasteur_status, bd=4, height=7, justify=LEFT, padx=100, relief=RAISED, text='Pasteur', font="Arial 15 bold ")
    l3.grid(row=1, column=2, padx=100, sticky=E)

    # Current Process Time
    pt = Label(bg='#DCDCDC', bd=4, height=2, justify=CENTER, relief=SUNKEN,
               text='Current Process Time: ' + str(round(env.now, 2)) + '  seconds', font="Arial 13 bold ")
    ...
    def update():
        l1['bg'] = depall_status
        l2['bg'] = filler_status
        l3['bg'] = pasteur_status
        pt['text'] = 'Current Simulation Time: ' + str(round(env.now, 2)) + '  minutes'
        window.after(70, update)  # in ms
